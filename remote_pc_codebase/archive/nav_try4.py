#!/usr/bin/env python3
"""
nav2_gap_nav.py  (integration-fixed + obstacle-fixed)
======================================================
...
Integration bug-fixes (this file):
  INT-BUG 1 — pause did NOT call _blacklist_goal(), so the cancelled goal
              ...
              Also: pause/resume were not idempotent; a duplicate pause would
              re-cancel an already-cancelled goal.
              Fixed: _nav_cmd_cb is fully idempotent; pause always blacklists
              the active goal before stopping.

  INT-BUG 2 — _on_nav_result fired while _paused=True (Nav2 returns
              STATUS_CANCELLED asynchronously after cancel_goal_async),
              flipping state to GAP_SELECT while paused. First _goal_tick
              after resume then sent a new Nav2 goal before dock finished.
              Fixed: _on_nav_result guards the state transition on _paused.

  INT-BUG 3 — A pause arriving while the nav node was in its own BACKING_UP
              state (stuck recovery) left it continuing to publish /cmd_vel,
              conflicting with the dock controller on the same topic.
              Fixed: pause handler aborts the stuck-backup immediately.

  INT-BUG 4 — _pre_pause_state was stored but always ignored on resume
              (resume unconditionally set GAP_SELECT). Dead variable removed.

Obstacle-fix history (this file):
  OBS-FIX 1 — INFLATION_R was 0.15 but YAML inflation_radius was 0.13.
              The Python blocked_grid used 0.15 m while Nav2 used 0.13 m,
              meaning Nav2's actual clearance was below the robot's half-width.
              Both are now 0.18 m (robot half-width 0.15 m + 3 cm margin).

  OBS-FIX 2 — Gap targets at short d_edge caused the lateral shift
              (math.asin(ROBOT_SIDE_M / d_edge)) to overflow toward 90 degrees,
              pushing the target into the opposite wall. Added a minimum d_edge
              guard of 0.25 m to skip unreliable close-range gap targets.

Nav fix history (pre-integration):
  FIX 1  — Info gain normalised to 0-1 ratio.
  FIX 2  — Heading cost penalises goals behind the robot.
  FIX 3  — Frontier scoring uses efficiency ratio dist/(ig+0.1).
  FIX 4  — Frontier centroid snapped to nearest actual frontier cell.
  FIX C  — Frontiers/gaps with ig_ratio < IG_RATIO_MIN skipped.
  FIX GAP MAP/BOUNDS — Gap targets validated against map + info gain.
  FIX TIMEOUT    — Flat 45s timeout.
  FIX BACKUP     — Rear-blocked backup nudges forward+rotate.
  FIX STUCK      — STUCK_DIST 0.40m.
  FIX BLACKLIST  — Cap at 60; expiry at 6× timeout.
  FIX RATES      — FRONTIER_HZ 2.0, CONTROL_HZ 10.0.
  FIX INFLATION  — INFLATION_R matches YAML inflation_radius (now both 0.18).
  FIX REGION     — 3 stucks in 1m area buries the frontier permanently.
"""
...
GAP_BUCKET_DEG  = 10
ROBOT_SIDE_M    = 0.17
# OBS-FIX 2: minimum d_edge before lateral-shift geometry becomes unreliable.
# Below 0.25 m, asin(ROBOT_SIDE_M / d_edge) approaches 90 deg and the computed
# target crosses to the opposite wall.
GAP_MIN_EDGE_D  = 0.25
...
OCC_THRESH      = 60
MAP_OCC_THRESH  = 60
# OBS-FIX 1: was 0.15, which differed from the YAML value of 0.13.
# Now set to 0.18 to match the corrected YAML inflation_radius.
# This constant controls the Python node's own blocked_grid filter —
# it must be >= the YAML value so this node is the stricter gate,
# ensuring gap/frontier targets rejected here are also safe in Nav2.
INFLATION_R     = 0.16
...
STUCK_DIST      = 0.40
STUCK_TIME      = 12.0
BACKUP_SPEED    = 0.08   # conservative backup speed regardless of vx_max change
BACKUP_DURATION = 1.8
...
class Nav2GapNav(Node):
    ...
    def __init__(self):
        ...
        # INT-BUG 4 FIX: _pre_pause_state removed (was stored but never used)
        self._paused = False
    ...
    def _nav_cmd_cb(self, msg: String):
        ...
        pause:
          ...
          - Aborts any in-progress stuck-backup (INT-BUG 3) to stop
            /cmd_vel publishing immediately.
          - Cancels any active Nav2 goal.
          - Blacklists the current frontier_goal so it will NOT be
            re-selected immediately on resume (this was the backtrack cause).
          - Transitions state to 'PAUSED' so _goal_tick and _control_tick
            both see a consistent non-NAVIGATING/BACKING_UP state.

        resume:
          - Guard: no-op if not paused.
          - Defensively clears frontier_goal (belt-and-braces).
        ...
        if cmd == 'pause':
            ...
            # INT-BUG 3 FIX: abort stuck-backup to stop /cmd_vel conflict
            ...
            # INT-BUG 1 FIX: blacklist so resume doesn't re-select same goal
            ...
            self._paused       = False
            self.frontier_goal = None  # defensive clear
            self.goal_start_t  = None
    ...
    def _rebuild_costmap(self):
        ...
        # OBS-FIX 1: INFLATION_R now 0.18 (was 0.15), matching the corrected YAML.
    ...
    def _select_gap(self, rx, ry, yaw):
        ...
        for i in range(n - 1):
            ...
            # OBS-FIX 2: skip gaps where the obstacle edge is too close.
            # At d_edge < GAP_MIN_EDGE_D the lateral shift angle
            # (asin(ROBOT_SIDE_M / d_edge)) approaches 90 degrees, meaning
            # the safe_angle points almost perpendicular to the intended
            # heading and the computed target lands in the opposite wall.
            ...
            shift_rad  = math.asin(min(1.0, ROBOT_SIDE_M / max(0.15, d_edge)))
            safe_angle = ray_angle + shift_rad if r1 < r2 else ray_angle - shift_rad
            ...
            score = W_ANGLE * angle_diff - W_GAP_WIDTH * jump + W_GAP_DIST * d_edge
    ...
    def _goal_tick(self):
        # Publish nav state heartbeat so coordinator can monitor.
        m = String()
    ...
    def _on_nav_result(self, future):
        ...
        Nav2 returns STATUS_CANCELLED asynchronously after cancel_goal_async.
        Without this guard the callback fires after _paused=True is set and
        transitions state to GAP_SELECT, causing the first _goal_tick after
        resume to send a new Nav2 goal before the dock controller has finished.
        """
        try:
        ...
        # INT-BUG 2 FIX: don't clobber PAUSED state with GAP_SELECT
